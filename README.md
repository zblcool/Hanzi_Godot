...
- `仓颉之路` 目前不再只是静态占位卡，而是有独立 portal overlay 预览 source 里的入口、卡牌、合字、遗物、战后奖励链路、节点与敌意样本；入口页还补上了可点按回应的对峙舞台与一个本地 `3D 特效` 开关，先把 source 的前台互动感迁回第一步；真正的 Godot 爬塔战斗仍待后续迁移接入
