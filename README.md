...
- `简库中庭 / Slip Archive` 里现在已经补上两道句阵守卫：`风雨同舟` 会给一段回气奖励，`海上生明月` 会唤出一名远程守句并额外给一段字墨收益，先把 source 里的 guarded phrase event 和非单一奖励类型继续往前接
...
- 成语 / 句子类地图事件现在已经接回两道 `句阵守卫`，并开始区分回气 / 字墨两种房间奖励；更广的句池、显形 / 偏旁等奖励分支和后续房间布置仍在迁移清单里继续追赶
