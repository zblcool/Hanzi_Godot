...
- `简库中庭 / Slip Archive` 里现在已经补上三道句阵守卫：`风雨同舟` 会给一段回气奖励，`海上生明月` 会唤出一名远程守句并额外给一段字墨收益，`山高水长` 则会带来一名重装守句并扩大附近迷雾显形，继续把 source 里的 guarded phrase event 和多种房间奖励往前接
...
- 成语 / 句子类地图事件现在已经接回三道 `句阵守卫`，并开始区分回气 / 字墨 / 显形三种房间奖励；更广的句池、偏旁等奖励分支和后续房间布置仍在迁移清单里继续追赶
