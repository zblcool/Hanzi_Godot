...
- `[done]` First post-boss room objective beyond wave clear. Notes: Godot now scatters three ward seals across `入卷前庭 / Entry Court` after the first scroll lord falls; the `卷间抉择` gate only opens once they are gathered, so the first chamber break depends on more than raw cleanup.
