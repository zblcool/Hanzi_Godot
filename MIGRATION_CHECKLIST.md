...
- `[in progress]` More front-end hero flavor beyond the archive pass. Notes: Godot now mirrors the source card-side quote bubble, selected marker, source-style scholar/xia avatar silhouettes, and a compact current-writer spotlight strip for opening cues / source-skill preview / primary route hint across the menu spotlight, hero cards, and archive panels alongside the existing selection quotes and battle-side callouts, but there are still extra front-portal framing beats that have not been ported.
