...
- `[done]` First post-boss room objective beyond wave clear. Notes: Godot now mirrors the source's newer sealed-exit cadence in `入卷前庭 / Entry Court`: a single `封` ward beacon rises after the first scroll lord falls, touching it spawns the named `砚门守魁 / Ink Gate Warden`, and only that ambush kill raises the actual `奖` reward beacon for the between-chambers choice.
