...
- `[in progress]` Move launcher / menu content into data-driven definitions. Notes: `scripts/core/front_end_content.gd` now drives launcher cards, launcher about/profile/changelog copy, Cangjie portal sections, menu shell copy, menu top actions, menu quick-start entries, menu main-detail hints, menu overlay headers/summaries/CTA copy, menu-side transition shell, and the first batch of archive section labels/notes; deeper archive card bodies and leaderboard formatting logic still remain inline in scene scripts.
