...
- `[in progress]` Cangjie portal follow-through. Notes: Godot launcher now gives `仓颉之路` a dedicated portal overlay that previews the source-style `Start Climb`, `Card Codex`, `Fusion Atlas`, `Relic Shelf`, `Post-Battle Flow`, and `Tower Guide`; a true playable Godot scene or direct prototype hand-off is still pending.
...
- `[in progress]` Phrase / idiom guardian encounters. Notes: `简库中庭 / Slip Archive` now has `风雨同舟`、`海上生明月`、`山高水长`、`举头望明月`, covering mixed guardian families and all current source phrase reward types, but the wider phrase pool and later-room guardian routing still need follow-through.
...
- `[pending]` Wider radical / recipe / word pool. Notes: Godot now carries 明 / 休 / 海 / 雷 / 忍 / 炎 plus their first word upgrades, but the source prototype still has broader branches such as 双日、林、沁、岩、囚、俊、强 waiting to migrate.
...
- `[in progress]` Dungeon-room transition and interlude flow. Notes: hanziHero now breaks longer runs into room-to-room reward / event / recovery beats after major milestones; Godot now has the first chamber-break continue gate, a chamber-end reward beacon, a live seal / reward guidance chip, one low-risk choice trio, a source-style next-room preview block, a dedicated continue-deeper overlay, a first carried chamber-modifier pass, one real named chamber reseed, two entry-court exit-objective branches (gatekeeper ward or two-seal relay), and four guarded phrase events, but relic-backed event outcomes and later chamber routing are still missing.
...
- `[in progress]` More front-end hero flavor beyond the archive pass. Notes: Godot now mirrors the source card-side quote bubble, selected marker, source-style scholar/xia avatar silhouettes, hero-accented current-writer preview tinting, a short selection pulse, and a quoted-thread caption / stage shell across the menu spotlight, hero cards, and archive panels alongside the existing opening cues / source-skill preview / primary route hint strip and battle-side callouts, but there are still extra front-portal framing beats that have not been ported.
