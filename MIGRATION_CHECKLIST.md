...
- `[in progress]` Phrase / idiom guardian encounters. Notes: the first `风雨同舟` guard event is now live in `简库中庭 / Slip Archive`, but the wider phrase pool, mixed guardian families, and later-room guardian routing still need follow-through.
...
- `[in progress]` More map-event reward routing. Notes: Godot now has a first guarded-phrase heal reward in `简库中庭 / Slip Archive`, but the broader XP / reveal / radical sentence outcomes still remain unported.
