...
- `[in progress]` Phrase / idiom guardian encounters. Notes: `简库中庭 / Slip Archive` now has both `风雨同舟` and `海上生明月`, including a first mixed guardian-family pass via a ranged sentence watcher, but the wider phrase pool and later-room guardian routing still need follow-through.
...
- `[in progress]` More map-event reward routing. Notes: Godot now carries both heal and `字墨` guarded-phrase rewards in `简库中庭 / Slip Archive`, but the broader reveal / radical sentence outcomes still remain unported.
