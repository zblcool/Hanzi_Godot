...
- `[in progress]` More front-end hero flavor beyond the archive pass. Notes: Godot now mirrors the source card-side quote bubble, selected marker, source-style scholar/xia avatar silhouettes, hero-accented current-writer preview tinting, and a short selection pulse across the menu spotlight, hero cards, and archive panels alongside the existing opening cues / source-skill preview / primary route hint strip and battle-side callouts, but there are still extra front-portal framing beats that have not been ported.
