...
- `[done]` First carried chamber modifier on the interlude event branch. Notes: Godot's `残卷回响` route now persists into the following chamber by biasing pressure-enemy drops toward extra `残纸` and letting elites echo a `疾书令`, so one interlude branch now behaves like a real room modifier instead of only an instant haste payout.
- `[in progress]` Dungeon-room transition and interlude flow. Notes: hanziHero now breaks longer runs into room-to-room reward / event / recovery beats after major milestones; Godot now has the first chamber-break continue gate, one low-risk choice trio, a source-style next-room preview block, and one carried chamber modifier, but actual room reseeding, relic-backed event outcomes, and later chamber routing are still missing.
